...
const ManageApplications = () => {
  ...
  const handleDeleteApplication = async (id) => {
    ...
    try {
      ...
      if (res?.success) {
        const deleteApp = (prevData) =>
          prevData.filter((application) => application.id !== id);
  ...
  const filterApplications = (status, scholarship) => {
    ...
    if (scholarship !== "All") {
      filtered = filtered.filter(
        (application) => application.scholarship.title === scholarship
      );
    }
  ...
  const uniqueScholarships = [
    "All",
    ...new Set(
      applications.map((application) => application.scholarship.title)
    ),
  ];
  ...
  return (
    <section className="p-8 bg-gray-900 min-h-screen">
      ...
      <div className="mb-6 flex justify-between">
        <select
          onChange={(e) =>
            handleFilterChange(e.target.value, scholarshipFilter)
          }
          value={statusFilter}
